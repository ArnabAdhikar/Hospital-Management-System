...
from face_recoginzer import *
class Hospital:
    def __init__(self, root):
        self.root = root
        self.root.title("Hospital Management System-> A.Anderson")
        ...
        # DataframeLeft
        lblNameTablet = Label(DataFrameLeft, text="Name of Tablet", font=("times new roman", 12, "bold"), padx=2,
                              pady=6)
    ...
    def update(self):
        ...
        my_cursor = conn.cursor()
        my_cursor.execute("update hospital set Nameoftablets=%s,Dose=%s,Numberoftablets=%s,Lot=%s,Issuedate=%s,"
                        "ExpDate=%s,DailyDose=%s,StorageAdvice=%s,NHSNumber=%s,PatientName=%s,DateOfBirth=%s,"
                        "PatientAddress=%s where ref=%s;",
            (
                self.Nameoftablets.get(),
                self.Dose.get(),
                self.Numberoftablets.get(),
    ...
    def fatch_data(self):
        ...
        if len(rows) != 0:
            ...
            conn.commit()
        conn.close()
    def get_cursor(self, event="", content=None):
